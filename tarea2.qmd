...
A continuación se preprocesarán las muestras, eliminando aquellas con datos faltantes. Se usarán los conteos por millón para reestructurar el tamaño de las librerías y normalizar asó los datos de conteo obtenidos por la secuenciación. También, se eliminarán los genes con conteos bajos:
...
datatable_report_glm_mut <- generate_gene_report(
  ...
  fdr_column = "FDR",
  title = "Evaluación del efecto de las mutaciones con un modelo lineal generalizado con edgeR"
)
...
informe_ora_hyp<-generate_ora_report(ora_hyp_glm_go, title = "Análisis de sobrerepresentación de grupos en hipoxia", threshold=0.05, analysis_type = "GO")

informe_ora_hyp
```

De estos 35 grupos sobrerepresentados, se representarán a continuación los 15 con más significancia y más sobrerepresentados de acuerdo a su *odds ratio*, mediante un gráfico de burbujas:
...
#| warning: false
# echo:false
# Este código no se mostrarís, dejando únicamente la salida
...
Por lo que hace a los mutantes, tal y como era esperable, se activan distintas vías alternativas que permiten tanto mantener el equilibrio redox, como el crecimiento celular.
...
En resumen, aunque no existe interacción entre hipoxia y mutantes para genes relacionados con ella, sí que se observan distintas vías adaptativas a las distintas condiciones, sobre todo en el caso del mantenimiento del equilibrio redox. Tanto en condiciones de hipoxia como al deplecionar distintos genes que permiten la adaptación de *Caenoharbditis elegans*, el equilibrio redox cambia y su reajuste es esencial para la supervivencia de estos animales.